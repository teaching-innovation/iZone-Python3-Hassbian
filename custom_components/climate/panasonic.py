...
#            value = 'off_command'
            jsonOn = {'SystemON':'off'}
            _LOGGER.info("Sending command [%s] to %s", jsonOn, self._name)
            _LOGGER.info("Sending command [%s] to %s", jsonOn, self._name)

        else: 
            jsonTemp = {'UnitSetpoint':'{0:g}'.format(float(self._target_temperature))}
            jsonMode = {'SystemMODE':self._current_operation.lower()}
            jsonFan = {'SystemFAN':self._current_fan_mode.lower()}
            jsonOn = {'SystemON':'on'}
            _LOGGER.info("Sending command [%s %s %s] to %s", jsonTemp, jsonMode, jsonFan, self._name)
            
            c = pycurl.Curl()
            c.setopt(c.URL, SetPointENDPOINT)
            c.setopt(pycurl.POST, 1)
            c.setopt(pycurl.POSTFIELDS, json.dumps(jsonTemp))
            c.perform()
            c.close()

            c = pycurl.Curl()
            c.setopt(c.URL, ModeENDPOINT)
            c.setopt(pycurl.POST, 1)
            c.setopt(pycurl.POSTFIELDS, json.dumps(jsonMode))
            c.perform()
            c.close()

            c = pycurl.Curl()
            c.setopt(c.URL, FanENDPOINT)
            c.setopt(pycurl.POST, 1)
            c.setopt(pycurl.POSTFIELDS, json.dumps(jsonFan))
            c.perform()
            c.close()
